...
var Game = function() {
	...
	this.swipeGems = function(a, aID, b, bID) {
		...
		console.log("Attempting switch: ", aID, bID);

		if (this.switchEnd && a !== undefined && b !== undefined && aID >= 0 && bID >= 0 && aID <= this.size && bID <= this.size) {
			
			if(!this.shouldSwipe(a,aID,b,bID)){
				console.log("Swiping Blocked : ");
				return;
			}
			console.log("switching: ", aID, bID);
			var that = this;
			var aTop = a.css('top');
	...
	this.shouldSwipe =  function(a,aID,b,bID){
		var level = 'N';
		var left = [], right = [] , up = [] . down = [];
		var position = bID;
		var directions = [];
		//console.log(a);
		//console.log("shouldSwipe: {[a = %s,aId = %s],[b = %s,bId = %s]}", a,aID, b,bID);
		//return false;
		//get el to the left
		switch(this.direction) {
			case "dragleft":
			directions = [
					this.elementsRight(position) ,
				 	this.elementsUp(position) , 
				 	this.elementsDown(position)
				 	];
			break;

			case "dragright":
			directions = [
					this.elementsLeft(position) ,
				 	this.elementsUp(position) , 
				 	this.elementsDown(position)
				 	];
			break;
		...
		for (var i = directions.length - 1; i >= 0; i--) {
			result = result | this.checkIfSameType(a.attr('class'), directions[i],this.minMatch);
			if(result){
				break;
			}
		}
	...
	this.elementsLeft =  function(position){
		var left = [];
		var el = null;
		for (var i = 1; i < this.originalSize; i++) {
			el = this.base.find('.row[data-id='+ (position - i) +']');
			if(!el){
				break;
			}
				left.push(el);
	...
	this.elementsRight = function(position){
		var right = [];
		var el = null;
		for (var i = 1; i < this.originalSize ; i++) {
			el = this.base.find('.row[data-id='+ (position + i) +']');
	...
	this.elementsUp = function(position){
		var up = [];
		var el = null ;
		for (var i = position; i >= 0 ; i-=this.originalSize) {
			el = this.base.find('.row[data-id='+ (i) +']');
			if(!el){
				break;
			}
			up.push(el);
	...
	this.elementsDown = function(position){
		var down = [];
		var el = null ;
		for (var i = position; i < this.size ; i+=this.originalSize) {
			el = this.base.find('.row[data-id='+ (i) +']');
			if(!el){
				break;
			}
	...
	//@@@
	this.checkIfSameType =  function(type,elArray,atLeast){
		var same = false;
		var matched = 0;
		if(!elArray.length){
			return true;
		...
		return (matched+1) == atLeast;
	};

	this.checkGemAround = function(gemType, position) {
		var flag = false;
		//console.log("checkGemAround : %s \t : \t %s\n",gemType, position);
